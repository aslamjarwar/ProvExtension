...
import src.prov.util as provUtil
...
class ProvBundle(object):
    ...
    _bundles = dict()
    _nestBrecords=list()
    #multiDict=provUtil.MultiBundle()
    trackBundles={}
    count=0
    closingBundle=""
    
    #def __init__(self, records=None, identifier=None, namespaces=None, document=None):
    def __init__(self, records=None, identifier=None, namespaces=None, document=None, attributes_env=None,bundle_root=None):
        """
        Constructor.
    ...
    @property
    def listOfBundles(self):
        """
        Returns the list of bundles
        """
        return list (self._nestBrecords)
    @property
    def bRoot(self):
        """
        Returns the bundle's attributes
        """
        return self._bundle_root
    @property
    def attributes_env(self):
        """
        Returns the bundle's attributes
        """
    ...
    def get_provn(self, _indent_level=0):
        ...
        xx=None
        if(self._attributes_env is not None):
            xx=encoding_provn_value(self._attributes_env)
            xx=xx.replace('{','[').replace('}',']')
            lines = ["document"] if self.is_document() else ["bundle(%s" % self._identifier+ ','+xx+')']
        else:
            lines = ["document"] if self.is_document() else ["bundle %s" % self._identifier]
                    
        
        #print("*****bundle(%s" % lines)
        ...
        return provn_str
# method for bundles 
    def get_provn_p(self, _indent_level=0):
        """
        Returns the PROV-N representation of the bundle.
...
# =============================================================================
        if self.is_document():
               # Print out bundles
               lines.extend(bundle.get_provn_p(_indent_level + 2) for bundle in self.bundles)
...
# =============================================================================
        ...
        provn_str = newline.join(lines) + "\n " 
        if(self.is_document()):
             provn_str += "endDocument"
        else:
            if (self.bundle_has_child(self.identifier)):
                provn_str += ""
            else:
                provn_str += "endBundle"
# =============================================================================
#         if(self.identifier not in self.trackBundles):
#             provn_str += "  endBundle \n"
#             
#         
#         provn_str += indentation
...
#              provn_str += "endDocument"
#         else:
#             if (self.is_bundle() and self.identifier not in self.trackBundles):
#                 #close +="endBundle \n"
#                  provn_str += "endBundle"
...
#                 print (self.count)
#                 if (self._identifier in self.trackBundles):
#                     self.count=len(self.trackBundles.get(self._identifier))
#                     for line in range(self.count):
#                         closingBundle+= "endBundle \n "               
#                     provn_str += closingBundle
#                     #self.tempIdentifier= count
#                 else:
#                     #self.count=0
#                     #provn_str += closingBundle
#                     #self.closingBundle=""
...
# =============================================================================
        ...
        return provn_str
    def bundle_has_child(self, identifier):
        for key, value in self.trackBundles.items():
            if (key==identifier):
                return True
        return False
    ...
    def bundle_p(self, identifier, other_attributes=None):
        """
        Creates a a nested bundle
        """
        b = ProvBundle(identifier=identifier, attributes_env=other_attributes, parent=self._identifier, document=self)
        self._nestBrecords.append(b)
        #ProvDocument._bundles[valid_id] = b
        return b
    def bundle_p_new(self, identifier, other_attributes=None,root=None):
         #print("in bundle")
        """
        ...
        b = ProvBundle(identifier=valid_id, attributes_env=other_attributes,bundle_root=root,document= self)         
        self._bundles[valid_id] = b        
        self._nestBrecords.append(b)
        #parent_id=self._bundle_root.identifier
        if (type(root) is ProvBundle):
            provUtil.add_values_in_dict(self.trackBundles,root.identifier,identifier)
            #print('identifier %s'% identifier)
            #self.multiDict[root.identifier]=identifier
            #for identifier,root.identifier in enumerate(multiDict):
               # if multiDict[identifier] not in d[key]: 
                
                
            
        else:
            ProvException("Invalid type of Bundle parent id .")
        return b
...
class ProvDocument(ProvBundle):
    ...
    def __init__(self, records=None, namespaces=None):
        ...
        """
        ProvBundle.__init__(
            self, records=records, identifier=None, namespaces=namespaces, attributes_env=None,bundle_root=None
        )
        #self._bundles = dict()
    ...
    def bundle(self, identifier,attributes=None):
        #print("in bundle")
        """
        Returns a new bundle from the current document.
        ...
        if valid_id in self._bundles:
            raise ProvException("A bundle with that identifier already exists")
        #b = ProvBundle(identifier=valid_id, document=self)
        b = ProvBundle(identifier=valid_id, attributes_env=attributes,parent='document',document=self)
         
        self._bundles[valid_id] = b
        return b
    def bundle_p(self, identifier,attributes=None):
        #print("in bundle")
        """
        Returns a new bundle from the current document.
        ...
        if valid_id in self._bundles:
            raise ProvException("A bundle with that identifier already exists")
        #b = ProvBundle(identifier=valid_id, document=self)
        b = ProvBundle(identifier=valid_id, attributes_env=attributes,bundle_root='document',document=self)
